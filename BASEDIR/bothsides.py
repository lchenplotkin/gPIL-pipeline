...
leftpath='1/leftbrains'
rightpath='1/rightbrains'
        ...
        for sectionfile in filedict[brainfolder]:
            with open(sectionfile,'r') as f:
                ...
                for line in templist:
                    if float(line[9])>0.0005 and line[1] not in artifactlist:
    ...
    for brain in braindict:
        ...
        for section in filedict[brain]: #ie. "For each csv in each folder"
            with open(section,'r') as f: 
                ...
                i=0
                for index in indexlist:
                    totalarea[i]+=float(templist[index][3])
                    i+=1
        ...
        for section in filedict[brain]:
            with open(section,'r') as f:
                ...
                i=0
                for index in indexlist:
                    name=templist[index][1]
                    if name not in namelist:
                        namelist.insert(i,name)
                    modifier=float(templist[index][3])/totalarea[i]
                    #some images have more or less of different regions present, so we take a weighted average

                    braindict[brain][i]+=float(templist[index][9])*modifier #applying weighted average
                    if float(templist[index][9])*modifier>maxload:
                        maxload=float(templist[index][9])*modifier#the maxload is used to scale on the colorbar
                    i+=1
...
leftrun1=prepare(leftpath,[])
leftkeys=leftrun1[0]
leftvals=leftrun1[1]
leftmax=leftrun1[2]
indlist=leftrun1[3]

'''rightrun=prepare(rightpath,indlist)
...
leftrun2=prepare(leftpath,indlist)
leftkeys=leftrun2[0]
leftvals=leftrun2[1]
leftmax=leftrun2[2]
#Must run again to use the same regions
'''
...
#plot_test([viridis,viridis],[leftvals,leftvals],namelist,leftkeys,"LEFT.png",[leftmax,leftmax])

plot_test([viridis,magma],[leftvals,rightvals],namelist,leftkeys,"BOTH.png",[leftmax,rightmax])
#exporting image
